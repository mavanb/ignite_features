...
class Trainer:
    ...
    def __init__(self, model, loss, optimizer, data_train, data_test, conf):
        ...
        self.train_loader, self.val_loader = self._get_train_valid_data(data_train, valid_size=conf.valid_size,
    ...
    @staticmethod
    def _get_train_valid_data(data_set, batch_size, seed=None, valid_size=0.1, shuffle=True, num_workers=4,
        ...
        if train_max:
            assert (train_max * batch_size < len(
                train_idx)), "train_max should be lower than number of samples in train set"
            train_idx = train_idx[:train_max * batch_size]
        if valid_max:
            assert (valid_max * batch_size < len(
                valid_idx)), "valid_max should be lower than number of samples in valid set"
